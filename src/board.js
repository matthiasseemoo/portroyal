      ...
      playerList.push(
        <div>
          <CardDisplayWithHeader cards={this.props.G.playerDisplays[i]} cardsOnClick={(cardIdx) => this.handlePlayerCardClick(this.props.G.playerDisplays[i][cardIdx])} overflow="auto" displayCards={this.state.displayPlayerId === i}>
            {(this.props.ctx.gameover) ? '' : ((parseInt(this.props.ctx.currentPlayer) === i) ? <span>&#x1f449;</span> : '')} <span onClick={() => this.setState({ displayPlayerId: i }) }>Player {i}</span> {(this.props.ctx.gameover) ? ((this.props.ctx.gameover.winners.includes(i)) ? <span>&#x1f3c6;</span> : '') : <span>{((parseInt(this.props.G.activePlayer) === i) ? <span>&#x1f34c;</span> : '')} {((parseInt(this.props.ctx.currentPlayer) === i) ? <span>({ (this.props.ctx.activePlayers !== null) ? this.props.ctx.activePlayers[i] : '' }{((parseInt(this.props.G.activePlayer) === i) ? <span onClick={() => this.props.moves.EndStage()}> &#x27A1;</span> : '')}, &#x1f590;: {this.props.G.drawCount})</span> : '')}</span>},
